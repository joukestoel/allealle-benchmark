...
//		return hole.function(Pigeon, Hole);
		final Formula domain = hole.in(Pigeon.product(Hole));

		final Variable h = Variable.unary("h");
		Formula func = hole.join(h).lone().forAll(h.oneOf(Hole));

		return domain.and(func);
	}
...
//				forAll(p1.oneOf(Pigeon).and(p2.oneOf(Pigeon)));
		...
		return p.join(hole).one().forAll(p.oneOf(Pigeon));
	}
...
//			solver.options().setSolver(SATFactory.ExternalSmtSolver);
