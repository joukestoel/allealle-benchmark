...
public class Knapsack extends AbstractChocoProblem {
    ...
    public void setUp() {
        // read capacities
        capacities = new int[]{data.data[0], data.data[1]};
        int no = data.data[2];
        if (n > -1) {
            no = n;
        }
        ...
        for (int i = 0, j = 3; i < no; i++) {
            ...
            nbOmax[i] = volumes[i]==0?42:(int) Math.ceil(capacities[1] / volumes[i]);
        }
    }
    ...
    public void buildModel() {
        ...
        IntVar scalar = model.intVar("weight", capacities[0] - 1, capacities[1] + 1, true);
